<?php
include_once __DIR__.'/ImageResizeException.php';
/**
...
//    public $quality_png = 6;
    ...
    private static function checkFormat($file)
    {
    ...
    public static function getWebp($file_src, $quality = null)
    {
        if ($quality === null) {
            $quality = 100;
        }
        ...
        $array['SRC'] = $file_src;
        $array['WEBP_FILE_NAME'] = $array['SRC'];

        if (self::checkFormat($_SERVER['DOCUMENT_ROOT'].$array['SRC']))
        {
            if (self::$isPng)
            {
                $array['WEBP_FILE_NAME'] = str_replace('.png', '.webp', strtolower($array['WEBP_FILE_NAME']));
            }
            else
            {
            ...
            $array['WEBP_PATH'] = explode('/', $_SERVER['DOCUMENT_ROOT'].$array['SRC']);
            array_pop($array['WEBP_PATH']);
            $array['WEBP_PATH'] = implode( '/', $array['WEBP_PATH']);

            var_dump('--WEBP_FILE_NAME--', $array['WEBP_FILE_NAME']);
            var_dump('--WEBP_PATH--', $array['WEBP_PATH']);


            if (!file_exists($_SERVER['DOCUMENT_ROOT'] . $array['WEBP_PATH']))
            {
                mkdir($_SERVER['DOCUMENT_ROOT'] . $array['WEBP_PATH'], 0755, true);
            }

            $array['WEBP_SRC'] = $array['WEBP_FILE_NAME'];

            if (!file_exists($_SERVER['DOCUMENT_ROOT'] . $array['WEBP_SRC']))
            {
                var_dump(' --isPng-- ', self::$isPng);
                if (self::$isPng)
                {
                ...
                imagewebp($im, $_SERVER['DOCUMENT_ROOT'] . $array['WEBP_SRC'], $quality);
                ...
                var_dump(' --WEBP_SRC-- ', $array['WEBP_SRC']);
            }

            $file_src = $array['WEBP_SRC'];
